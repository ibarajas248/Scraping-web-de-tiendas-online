# app.py
# Retail Analytics – Streamlit (conexión por túnel SSH o directa)
# Ejecutar en local (con túnel por código):
#   streamlit run app.py
# Ejecutar en el VPS (conexión directa, sin túnel):
#   USE_SSH_TUNNEL=0 streamlit run app.py --server.address 127.0.0.1 --server.port 8090 --server.headless true
...
# ---------- Flags de entorno ----------
# Por defecto, al correr en tu PC usa túnel SSH hacia el VPS.
USE_SSH_TUNNEL = os.getenv("USE_SSH_TUNNEL", "1") == "1"

# ---------- Credenciales SSH (para túnel desde local -> VPS) ----------
...
# ---------- Conexión SQLAlchemy con/ sin túnel ----------
@st.cache_resource(show_spinner=False)
def get_engine():
    """
    Crea el engine de SQLAlchemy.
    - USE_SSH_TUNNEL=1 (default): abre túnel SSH y conecta a la DB detrás del VPS.
    - USE_SSH_TUNNEL=0: conexión directa (útil si corres DENTRO del VPS).
    """
    # Si tu DB NO está en el mismo VPS del SSH, pon aquí el host/puerto reales de la DB
    REMOTE_DB_HOST = DB_HOST  # p.ej: "bnlrjjogqqtrux5dkhx3-mysql.services.clever-cloud.com"
    REMOTE_DB_PORT = DB_PORT  # p.ej: 3310

    def _make_mysql_creator(host, port):
        import mysql.connector
        return lambda: mysql.connector.connect(
            host=host,
            port=port,
            user=DB_USER,
            password=DB_PASS,
            ...
            # Habilita TLS si tu proveedor lo exige. Ideal: pasar ssl_ca y verificar.
            ssl_disabled=False,              # <- importante si el servidor fuerza TLS
    ...
    if USE_SSH_TUNNEL:
        try:
            ...
            st.error("Falta sshtunnel. Instala con: pip install sshtunnel")
            st.stop()

        # Abrimos túnel SSH hacia el VPS; del lado remoto nos conectamos al host:puerto de la DB
        ...
        tunnel.start()
        st.session_state["_ssh_tunnel"] = tunnel
        atexit.register(lambda: tunnel.stop() if tunnel.is_active else None)

        local_port = tunnel.local_bind_port
        creator = _make_mysql_creator("127.0.0.1", local_port)

        engine = create_engine(
            "mysql+mysqlconnector://",
            creator=creator,
            pool_pre_ping=True,   # valida conexiones antes de usarlas
            pool_recycle=1800,    # evita conexiones viejas
        )

    else:
        # Conexión directa (correr dentro del VPS o si el puerto está expuesto)
        url = (f"mysql+mysqlconnector://{DB_USER}:{DB_PASS}"
    ...
    try:
        ...
        st.error(f"No pude abrir la conexión a MySQL. Revisa host/puerto/SSL/túnel. Detalle: {e}")
        raise
    return engine
...
def vista_analisis():
    ...
    # (Aquí puedes agregar consultas/visualizaciones específicas para esta vista)
...
def vista_resultados():
    ...
    # (Aquí puedes preparar exportaciones/informes finales con su propio sidebar)
