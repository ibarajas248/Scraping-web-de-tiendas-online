...
st.write(
    "Cada tienda se procesa **secuencialmente** (una solicitud a la vez) para recorrer **toda la lista**. Puedes ejecutar varias tiendas en paralelo.")
...
def vtex_buscar_por_ean_session(session: requests.Session, base_url: str, ean: str, sc: Optional[str]) -> Tuple[
...
if archivo is not None:
    ...
    if st.button("Generar reporte consolidado"):
        ...
        cols_presentes = [c for c in order_cols if c in df_out.columns] + [c for c in df_out.columns if
                                                                           c not in order_cols]
        if not df_out.empty:
