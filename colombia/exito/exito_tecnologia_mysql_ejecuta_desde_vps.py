...
def insert_historico(cur, tienda_id: int, producto_tienda_id: int, p: Dict[str, Any], capturado_en: datetime):
    ...
    """, (
        ...
        tipo_oferta,
        None,
        p.get("precio_tachado_texto") or None,
        p.get("precio_texto") or None,
        p.get("vendedor") or None,
    ))

# -------------------------
# MySQL tunnel conn
# -------------------------
def get_conn_tunnel(db_host: str, db_port: int, db_user: str, db_pass: str, db_name: str):
    return mysql.connector.connect(
        host=db_host,
        port=db_port,
        user=db_user,
        password=db_pass,   # ← aquí puede ser "" y funciona si root no tiene password
        database=db_name,
        autocommit=False,
...
def main():
    ...
    ap.add_argument("--db-host", type=str, default="127.0.0.1")
    ap.add_argument("--db-port", type=int, default=3311)
    ap.add_argument("--db-name", type=str, default="analisis_retail")
    ap.add_argument("--db-user", type=str, default="root")

    # ✅ password vacío por defecto (NO pregunta nunca)
    ap.add_argument("--db-pass", type=str, default="")
    ...
    with sync_playwright() as p:
        ...
        def route_handler(route):
            ...
            if not args.include_images and rt in ("image", "media", "font"):
                return route.abort()
            if rt == "stylesheet":
                return route.abort()
    ...
    conn = None
    try:
        conn = get_conn_tunnel(args.db_host, args.db_port, args.db_user, args.db_pass, args.db_name)
        cur = conn.cursor()
        ...
        conn.commit()
        print(f"💾 Guardado en MySQL (túnel): {insertados} filas histórico para {TIENDA_NOMBRE} ({capturado_en_str})")
