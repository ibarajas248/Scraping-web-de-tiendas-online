...
"""
La Genovesa (ALMACÉN) – Scraper con Selenium + BeautifulSoup + Persistencia MySQL (versión robusta)

Mejoras clave:
- Cosecha incremental en vivo (no se basa en page_source final) para evitar pérdidas por virtualización.
- Detección y click de botones "Ver más / Mostrar más" con más patrones.
- Scroll sobre el contenedor REAL + bring-into-view del último card para disparar IntersectionObserver.
- Poll adaptable (timeout dinámico) y ráfagas adicionales controladas.
- Fallback a paginación por querystring (?page=N) si el infinito no crece.
- Aceptación modal de sucursal con verificación de cookie.
- Persistencia MySQL por chunks con reintentos ante 1205/1213.
- CLI con flags para headless, perfiles y outfile.
...
# Selectores de listado (ajustados; las tarjetas son los bloques del grid)
LIST_CARD_SEL   = "div.columContainerList"            # contenedor de cada ítem en listado
...
# ===== Tiempos / Scroll =====
WAIT = 30
SCROLL_MAX_ROUNDS = 400
SCROLL_MAX_NO_GROWTH_BURSTS = 10
...
def try_accept_location_modal(driver) -> bool:
    ...
    for c in closes:
        try:
            ...
            pass
    # Verificación de cookie/estado (best-effort)
    try:
        cookies = {c['name']: c['value'] for c in driver.get_cookies()}
        if not any(k for k in cookies if 'sucursal' in k.lower() or 'zona' in k.lower()):
            log("[MODAL] Aviso: no se detectó cookie de sucursal/zona (puede no ser necesaria).")
    except Exception:
        pass
    return acted
...
def try_click_load_more(driver) -> bool:
    xpaths = [
        ...
        "//*[contains(@class,'btn') and contains(.,'Más')]",
        "//*[contains(@role,'button') and (contains(.,'más') or contains(.,'Más'))]",
    ]
    for xp in xpaths:
...
def scroll_last_card_into_view(driver, item_selector: str):
    try:
...
def _poll_for_growth(driver, item_selector: str, prev_count: int, prev_height: int, root_el, rounds: int) -> Optional[Tuple[int, int]]:
    # Espera adaptable: a mayor cantidad de rondas, más paciencia
    base = 3.5
    extra = min(5.0, max(0.0, (rounds // 20) * 1.0))
    deadline = time.time() + base + extra
    while time.time() < deadline:
        human_sleep(0.20, 0.28)
...
def harvest_visible_cards(driver, seen: set) -> List[Dict]:
    out: List[Dict] = []
    cards = driver.find_elements(By.CSS_SELECTOR, LIST_CARD_SEL)
    for c in cards:
        try:
            link_el = c.find_element(By.CSS_SELECTOR, CARD_LINK_SEL)
            href = link_el.get_attribute("href")
            articulo_id = None
            if href and "ArticuloID=" in href:
                qs = parse_qs(urlparse(href).query)
                articulo_id = (qs.get("ArticuloID") or [None])[0]
            key = articulo_id or href
            if not key or key in seen:
                continue

            try:
                title = c.find_element(By.CSS_SELECTOR, CARD_TITLE_SEL).text.strip()
            except Exception:
                title = ""
            try:
                price_txt = c.find_element(By.CSS_SELECTOR, CARD_PRICE_SEL).text
            except Exception:
                price_txt = ""
            price_num = parse_price(price_txt)
            try:
                img_el = c.find_element(By.CSS_SELECTOR, CARD_IMG_SEL)
                img_src = img_el.get_attribute("src") if img_el else None
            except Exception:
                img_src = None
            try:
                stock_txt = c.find_element(By.CSS_SELECTOR, CARD_STOCK_TEXT).text.strip()
            except Exception:
                stock_txt = ""
            try:
                stock_dot = c.find_element(By.CSS_SELECTOR, CARD_STOCK_DOT)
                stock_color = stock_dot.get_attribute("style") if stock_dot else ""
            except Exception:
                stock_color = ""

            row = {
                "list_title": title,
                "list_price_text": price_txt,
                "list_price": price_num,
                "list_img": urljoin(BASE, img_src) if img_src else None,
            ...
            continue
    if out:
        log(f"[HARVEST] +{len(out)} nuevos (acum keys={len(seen)}).")
    return out


# ============== Scroll principal con crecimiento ==============

def scroll_until_no_more(driver, item_selector: str) -> List[Dict]:
    # Asegurar que el grid aparece
    ...
    seen_keys: set = set()
    harvest: List[Dict] = harvest_visible_cards(driver, seen_keys)

    while rounds < SCROLL_MAX_ROUNDS:
        rounds += 1

        # 1) Botón load more si existe
        if try_click_load_more(driver):
            scroll_last_card_into_view(driver, item_selector)
            grew = _poll_for_growth(driver, item_selector, prev_count, prev_height, root_el, rounds)
            if grew:
                prev_count, prev_height = grew
                same_rounds = 0
                no_growth_bursts = 0
                harvest.extend(harvest_visible_cards(driver, seen_keys))
                continue
        ...
        log(f"[SCROLL] Sin nuevos items (chequeo {same_rounds}/3) — total {cur}.")

        if same_rounds >= 3:
            no_growth_bursts += 1
            log(f"[SCROLL] Ráfaga extra sin crecimiento ({no_growth_bursts}/{SCROLL_MAX_NO_GROWTH_BURSTS}).")
            same_rounds = 0

            _scroll_burst_gentle(driver, item_selector, root_el)
            scroll_last_card_into_view(driver, item_selector)
            grew2 = _poll_for_growth(driver, item_selector, prev_count, prev_height, root_el, rounds)
            if grew2:
...
def parse_detail_page(driver, url: str) -> Dict:
    ...
    log(f"[DETAIL] EAN={ean or '-'} | {title[:60]} | {price_text or '-'} | {url}")
...
def ensure_listing_ready(driver, url: str):
    ...
    for attempt in range(1, 3):
        try:
            WebDriverWait(driver, WAIT).until(
                EC.presence_of_all_elements_located((By.CSS_SELECTOR, LIST_CARD_SEL))
            )
            cards = driver.find_elements(By.CSS_SELECTOR, LIST_CARD_SEL)
            if cards:
...
def scrape_categoria(
    ...
    try:
        ...
        # Scroll + Harvest incremental
        log("[INIT] Iniciando scroll/harvest…")
        harvest = scroll_until_no_more(driver, LIST_CARD_SEL)
        log(f"[LIST] Total cosechado del listado: {len(harvest)}")

        # Detalles
        rows: List[Dict] = []
        for i, card in enumerate(harvest, 1):
            ...
            while tries < 2:
                try:
                    ...
                    rows.append(row)
                    log(f"[OK] ({i}/{len(harvest)}) Guardado.")
                    break
                except TimeoutException:
                    tries += 1
