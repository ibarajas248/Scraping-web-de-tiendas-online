...
def parse_detail_page(driver, url: str = None) -> Dict:
    """
    Parsea la página de detalle ACTUAL (ya abierta por CLICK).
    url se usa solo para logging si se pasa.
    """
    WebDriverWait(driver, WAIT).until(
    ...
    # EAN en:
    # <div class="col textDescripcionDetalle" style="text-align:center"> 7790895009211 </div>
    ean = None
...
def scrape_categoria(
    ...
    try:
        ...
        # Detalles (CON CLICK EN CADA PRODUCTO)
        rows: List[Dict] = []
        ...
        for i, card in enumerate(cards, 1):
            ...
            while tries < 2:
                try:
                    # Re-buscar el link en el DOM por ArticuloID (más robusto que por texto)
                    if articulo_id:
                        xpath = f"//a[contains(@href, 'ArticuloID={articulo_id}')]"
                    else:
                        # Fallback: buscar por fragmento de href
                    ...
                    # Ahora estamos en la página de detalle: parsear (incluyendo EAN)
                    ...
                    # No re-scroll completo: se asume que el navegador mantiene el scroll.
                    break
...
def row_to_db_product(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "sku":        clean(sku),
        "record_id":  None,
        "ean":        clean(row.get("ean")),   # EAN se pasa a la capa DB
        "nombre":     nombre_det,
        "marca":      None,
        "fabricante": None,
