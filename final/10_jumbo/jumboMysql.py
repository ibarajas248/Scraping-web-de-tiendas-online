...
"""
Jumbo AR (VTEX) — Scraper completo por árbol de categorías.
Guarda:
- tiendas (upsert por codigo)
- productos (deduce categoria/subcategoria desde 'categories')
- producto_tienda (SKU/record/url/nombre_tienda)
- historico_precios (precios como texto VARCHAR y promos en JSON)
...
# ========= Helpers de limpieza =========
ILLEGAL_XLSX = re.compile(r'[\x00-\x08\x0B\x0C\x0E-\x1F]')  # si luego exportaras XLSX
...
def req_json(url: str, session: requests.Session, params: Optional[dict] = None) -> Optional[Any]:
    last = None
...
def to_txt_or_none(x) -> Optional[str]:
    ...
    if v is None:
        return None  # guardamos como texto compatible con VARCHAR
    return f"{round(float(v), 2)}"

# ========= Árbol de categorías =========
def get_category_tree(session: requests.Session, depth: int = TREE_DEPTH) -> List[dict]:
    url = f"{BASE}/api/catalog_system/pub/category/tree/{depth}"
    data = req_json(url, session)
    return data or []

def iter_paths(tree: List[dict]) -> List[str]:
...
# ========= Parse de producto (a filas por SKU y vendedor) =========
    ...
    for it in items:
        ...
        if not sellers:
            rows.append({
                ...
                "spotPrice": None,
                "name": name,
                "skuName": clean_text(it.get("name") or ""),
                "brand": brand,
                "ean": ean,
...
def fetch_category(session: requests.Session, cat_path: str) -> List[Dict[str, Any]]:
    rows: List[Dict[str, Any]] = []
    empty_streak = 0
    offset = 0
...
def insert_historico(cur, tienda_id: int, producto_tienda_id: int, row: Dict[str, Any], capturado_en: datetime) -> None:
    ...
    cur.execute(
        ...
        (
            ...
            producto_tienda_id,
            capturado_en,
            to_txt_or_none(row.get("listPrice")),
            to_txt_or_none(precio_oferta),
            None,                 # tipo_oferta (VTEX no lo expone directo)
            promo_tipo,
            None,                 # promo_texto_regular (p.ej. cuotas)
            None,                 # promo_texto_descuento
            comentarios
        )
    )
...
def main() -> None:
    ...
    try:
        ...
        print(
            f"\n💾 Guardado en MySQL: {total_insertados} filas de histórico "
            f"para {TIENDA_NOMBRE} ({capturado_en})"
        )
