...
# ⚠️ Recomendado: setear por ENV y NO hardcodear credenciales:
#   export DISCO_USER="..."
#   export DISCO_PASS="..."
DISCO_USER  = os.getenv("DISCO_USER", "comercial@factory-blue.com").strip()
DISCO_PASS  = os.getenv("DISCO_PASS", "Compras2025").strip()

PROVINCIA     = os.getenv("DISCO_PROVINCIA", "CABA").strip()
TIENDA_NOM    = os.getenv("DISCO_TIENDA", "Disco Alto Palermo").strip()
...
TIENDA_CODIGO = "Disco CABA"
TIENDA_NOMBRE = "Disco"
TIENDA_REF    = os.getenv("DISCO_REF_TIENDA", "Disco Alto Palermo")
...
# =========================
# ✅ NUEVO: detectar "Producto sin stock"
# =========================
def _has_out_of_stock_flag(driver) -> bool:
    """
    Detecta el flag VTEX de 'Producto sin stock' en el PDP.
    Basado en la clase estable: vtex-outOfStockFlag__wrapper / vtex-outOfStockFlag__text
    y en el texto 'Producto sin stock'.
    """
    # 1) wrapper estable
    try:
        if driver.find_elements(By.XPATH, "//*[contains(@class,'vtex-outOfStockFlag__wrapper')]"):
            return True
    except Exception:
        pass

    # 2) texto estable + texto “producto sin stock” (case-insensitive + acentos)
    try:
        xp = (
            "//*[contains(@class,'vtex-outOfStockFlag__text') and "
    ...
    # 3) fallback: cualquier nodo con el texto
    try:
        xp = (
            "//*[contains(translate(normalize-space(.),"
            "'ABCDEFGHIJKLMNOPQRSTUVWXYZÁÉÍÓÚ',"
...
def _scrape_pdp(driver, wait, pdp_url_rel: str) -> dict:
    ...
    # ✅ disponibilidad: si aparece el flag "Producto sin stock", no disponible
    disponible = not _has_out_of_stock_flag(driver)
...
def run_scrape_and_persist():
    ...
    try:
        ...
        try:
            ...
            for p in data:
                # ✅ CLAVE: si aparece "Producto sin stock" => NO INSERTA
                if p.get("disponible") is False:
                    skipped_sin_stock += 1
                    continue
                ...
                if insertados % 50 == 0:
                    conn.commit()
                    logging.info("💾 Commit intermedio: %d filas… (skip sin stock: %d)", insertados, skipped_sin_stock)

            conn.commit()
            logging.info(
                "✅ Guardado en MySQL: %d filas (skip sin stock: %d) para %s (%s)",
                insertados, skipped_sin_stock, TIENDA_NOM, capturado_en
            )
