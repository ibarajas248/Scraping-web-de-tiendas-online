...
def parse_vtex_bool(v):
    """Interpreta True/False aunque venga como string 'true'/'false' o 0/1.
    ...
    if isinstance(v, str):
        t = v.strip().lower()
        if t in ("true", "1", "t", "yes", "y", "si", "sí", "s"):
            return True
...
def persist_carrefour_df_to_mysql(df: pd.DataFrame):
    ...
    # --- Filtro extra: si el DF trae columna de disponibilidad, NO insertes cuando sea False ---
    for _col in ("IsAvailable", "isAvailable", "Disponible", "disponible", "available", "Available"):
        if _col in df.columns:
            df = df[df[_col].apply(lambda v: parse_vtex_bool(v) is not False)].copy()
            break
    if df.empty:
        print("⚠️ Todas las filas quedaron fuera por disponibilidad=False; nada que insertar.")
        return
...
if __name__ == "__main__":
    ...
    res = fetch_all_categories(depth=10)

    # Si el scraper devolvió tupla, tomar el primer elemento (normalmente el DF principal)
    ...
    if df is None or not isinstance(df, pd.DataFrame) or df.empty:
        print("No se obtuvieron productos (DF vacío o no retornó DataFrame).")
    else:
