...
# para detectar div.OferProd aunque el DOM esté raro
_RE_OFERPROD = re.compile(r'\bclass\s*=\s*["\'][^"\']*\bOferProd\b', re.IGNORECASE)
...
def list_products_on_page(html, base_url):
    ...
    for li in soup.select("li.cuadProd"):
        ...
        # ✅ CLAVE: detectar oferta en LISTADO
        has_offer_hint = (li.select_one("div.OferProd") is not None) or bool(_RE_OFERPROD.search(str(li)))
        ...
        out.append(
            {
                ...
                "nombre_list": nombre,
                "precio_list": precio,
                "has_offer_hint": has_offer_hint,  # ✅
            }
        )
    return out

def parse_detail(html, detail_url, ean_hint="", has_offer_hint=False):
    """
    REGLA:
    - Si hay div.OferProd (aunque esté vacío) => precio_lista = NULL y precio_oferta = precio_actual
    - Si no hay => precio_lista = precio_actual y precio_oferta = precio_actual
    Además: si el listado marca oferta, forzamos oferta aunque el detalle no lo muestre.
    """
    soup = BeautifulSoup(html, "html.parser")
    pr = ""
    try:
    ...
    # ✅ Detectar oferta (DOM o regex) + hint del listado
    has_offer_detail = (soup.select_one("div.OferProd") is not None) or bool(_RE_OFERPROD.search(html))
    has_offer = bool(has_offer_detail or has_offer_hint)
...
def scrape_one_category(url: str, session: requests.Session):
    ...
    while True:
        ...
        for it in items:
            ...
            # ✅ pasar hint de oferta desde listado
            det = parse_detail(
                rd.text,
                durl,
                ean_hint=it.get("ean_hint", ""),
                has_offer_hint=it.get("has_offer_hint", False),
            )
            ...
            # fallback precio si detalle vino vacío (respeta regla de oferta)
            if (det.get("Precio Oferta") is None or det.get("Precio Oferta") == 0) and it.get("precio_list"):
                det["Precio Oferta"] = it["precio_list"]

            # ✅ REGLA FINAL: si es OFERTA => precio_lista SIEMPRE NULL
            if det.get("Tipo Oferta") == "Oferta":
                det["Precio Lista"] = None
            else:
                # si NO es oferta y no hay lista/oferta, completar con precio del listado
                if (det.get("Precio Oferta") is None or det.get("Precio Oferta") == 0) and it.get("precio_list"):
                    det["Precio Oferta"] = it["precio_list"]
                if (det.get("Precio Lista") is None or det.get("Precio Lista") == 0) and it.get("precio_list"):
                    det["Precio Lista"] = it["precio_list"]

            row = {
                "EAN": det.get("EAN", "") or "",
                "Código Interno": det.get("Código Interno", "") or "",
                "Nombre Producto": det.get("Nombre Producto") or it.get("nombre_list", "") or "",
                "Categoría": cat_code,
                "Subcategoría": "",
                "Marca": det.get("Marca", "") or "",
                "Fabricante": "",
                "Precio de Lista": det.get("Precio Lista"),
                "Precio de Oferta": det.get("Precio Oferta"),
                "Tipo de Oferta": det.get("Tipo Oferta") or "Precio regular",
                "URL": det.get("URL", durl) or durl,
            }

            # print safe
            pl = row.get("Precio de Lista")
            po = row.get("Precio de Oferta")
            pl_txt = "NULL" if pl is None else f"{float(pl):.2f}"
            po_txt = "NULL" if po is None else f"{float(po):.2f}"
            print(f"  🛒 {row['EAN']} | {row['Nombre Producto']} | L:{pl_txt} O:{po_txt} | {row['URL']}")
