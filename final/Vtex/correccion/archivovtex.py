...
CAMBIO solicitado:
- NO insertar en MySQL los registros que NO estén disponibles (IsAvailable != true o AvailableQuantity <= 0),
  PERO sí reportarlos en el XLSX.
...
# ============== NUEVO: IsAvailable robusto (bool + string + variantes key) ==============
...
def _get_is_available_raw(co: Dict[str, Any]):
    for k in ("IsAvailable", "isAvailable", "isavailable"):
        if k in co:
            return co.get(k)
    return None

def _available_qty_int(co: Dict[str, Any]) -> int:
    v = co.get("AvailableQuantity")
    try:
        if v is None:
            return 0
        return max(0, int(v))
...
def simulate_price(session: requests.Session, base_url: str, sku_id: str, seller_id: str, sc: Optional[str], postal_code: str, country: str) -> Tuple[Optional[float], Optional[float]]:
    """Checkout simulation: retorna (lista, oferta) en float o (None, None)."""
    try:
        host = urlparse(base_url).netloc or base_url
        session.headers["Referer"] = f"https://{host}/"
        ...
        payload = {
            "items": [{"id": str(sku_id), "quantity": 1, "seller": str(seller_id or "1")}],
...
def parsear_producto_vtex(producto: Dict[str, Any], ean_consultado: str, base_url: str) -> List[Dict[str, Any]]:
    ...
    Cada fila corresponde a una combinación SKU/Seller.
    """
    filas = []
    ...
    for it in items:
        ...
        for s in sellers:
            ...
            # ===== NUEVO: disponibilidad real (IsAvailable bool/string + qty) =====
            is_av = _truthy_bool(_get_is_available_raw(co))
            qty = _available_qty_int(co)
            disponible = qty if is_av and qty > 0 else 0
            ...
            filas.append({
                ...
                "categoria": cat1, "subcategoria": cat2,
                "url": (base_url + link) if link and isinstance(link, str) and link.startswith("/") else link,
                "precio_lista": lista, "precio_oferta": oferta, "disponible": disponible,
...
def ingest_to_mysql(df: pd.DataFrame, batch_size: int = 100):
    """
    Ingresa el DataFrame resultante en MySQL.
    CAMBIO: filtra y SOLO inserta filas "disponibles" (disponible > 0).
    """
    ...
    try:
        ...
        for (supername, base), df_g in df.groupby(grp_cols):
            ...
            df_g = df_g.copy()
            if "disponible" in df_g.columns:
                disp = pd.to_numeric(df_g["disponible"], errors="coerce").fillna(0)
                df_g = df_g[disp > 0]
            else:
                df_g = df_g.iloc[0:0]
        ...
        print(f"[DB] ✅ Inserciones/updates en historico_precios (solo disponibles): {total}")
...
def main():
                    ...
                    for prod in productos:
                        ...
                        for f in filas:
                            ...
                            rows.append({
                                "supermercado": store.get("name", base),
                                "dominio": base,
                                # ✅ OK vs OK_SIN_STOCK según disponible
                                "estado_llamada": "OK" if disp > 0 else "OK_SIN_STOCK",
                                "ean_consultado": ean,
    ...
    if not df_out.empty:
        print("[DB] Guardando en MySQL… (solo disponibles)")
