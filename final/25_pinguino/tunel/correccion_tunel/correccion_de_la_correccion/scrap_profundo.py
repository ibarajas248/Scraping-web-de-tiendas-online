...
def fetch_products_by_dep(
    ...
    """
    Si 'page' viene con valor (int), trae SOLO esa página.
    Si 'page' es None, recorre TODAS las páginas de ese dep/cat
    hasta que una devuelva 0 productos.
    """
    all_prods: List[Dict[str, Any]] = []

    def _fetch_one(p: Optional[int], debug_suffix: str = "") -> List[Dict[str, Any]]:
        ...
        if save_debug:
            if debug_suffix:
                debug_name = save_debug.with_name(
                    save_debug.stem + debug_suffix + save_debug.suffix
                )
            else:
                debug_name = save_debug
            debug_name.write_text(html, encoding="utf-8")

        return parse_product_cards_enriched(
            html, dep_id, cat_id, dep_name, cat_name
        )

    # MODO 1: page especificado → solo esa página (útil para debug puntual)
    if page is not None:
        return _fetch_one(page)

    # MODO 2: sin page → barrer todas las páginas secuenciales
    # Página inicial (sin "pag")
    prods = _fetch_one(None)
    if not prods:
        return []
    all_prods.extend(prods)

    # Páginas siguientes: pag=2,3,4...
    current_page = 2
    while True:
        time.sleep(sleep)
        prods = _fetch_one(current_page, debug_suffix=f"_pag_{current_page}")
        if not prods:
            break
        all_prods.extend(prods)
        current_page += 1
...
def main():
    ...
    ap.add_argument("--sleep", type=float, default=1.2, help="Espera (seg) entre deptos/categorías/páginas")
    ...
    if args.only_ofertas:
        ...
        for dep in deps:
            ...
            try:
                ...
                print(f"[dep {dep_id}] productos (todas las páginas dep): {len(prods)}")
                rows.extend(prods)
                ...
                for cat in cats:
                    ...
                    try:
                        ...
                        print(f"[dep {dep_id} cat {cat_id}] productos (todas las páginas): {len(cat_prods)}")
