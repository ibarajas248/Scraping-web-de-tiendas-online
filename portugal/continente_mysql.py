...
PROXY_PORT = 823
PROXY_USER = "a1d102f8514e7ff8eea7__cr.pt"
...
def _infer_cat_from_url(url: str):
    # para este caso: /frescos/queijos/
    try:
        m = re.search(r"/([^/]+)/([^/]+)/", url)
        if not m:
            return ("", "")
...
def extract_all_tiles(page, page_url_for_cat: str):
    ...
    for i in range(n):
        ...
        rows.append({
            ...
            # --- campos para MySQL (tu estándar) ---
            "ean": None,  # Continente no lo da en tile
            "sku": pid or None,  # ✅ SKU = pid
            "record_id": pid or idx or None,  # fallback
            "nombre": name or "",
            "marca": brand or "",
            "fabricante": "",  # no disponible en tile
            "categoria": cat or "",
            "subcategoria": sub or "",
            "precio_lista": price_float,
            "precio_oferta": price_float,  # no tenemos promo clara acá
            "tipo_oferta": None,
            "promo_tipo": None,
...
def main():
    ...
    with sync_playwright() as pw:
        ...
        try:
            ...
            insertados = 0
            for p in rows:
                # EAN vacío (None) y clave por SKU (pid)
