...
def wait_results_loaded(page, timeout=60000):
    # 1) que exista el contenedor
    page.wait_for_selector("#delivery-area", state="attached", timeout=timeout)

    # 2) esperar a que deje de estar oculto (d-none) O que aparezcan resultados
    ok = False
    try:
        page.wait_for_function(
            """
            () => {
              const el = document.querySelector("#delivery-area");
              if (!el) return false;
              const hidden = el.classList.contains("d-none");
              const hasResults = document.querySelectorAll("#delivery-area .store-details.search-postal-code").length > 0;
              const hasAnyStore = document.querySelectorAll("#delivery-area label.store-label, #delivery-area input[name='customerAddress']").length > 0;
              return (!hidden && (hasResults || hasAnyStore));
            }
            """,
            timeout=timeout
        )
    ...
    # 3) fallback: a veces el resultado queda en el modal, o hay mensaje de error
    # espera un poco por algún mensaje típico
    try:
        page.wait_for_selector(
            "#coverage-area-modal .alert, #coverage-area-modal .error, #coverage-area-modal .form-error, "
            "#delivery-area .alert, #delivery-area .error, #delivery-area .form-error",
            timeout=5000
        )
        msg = page.locator(
            "#coverage-area-modal .alert, #coverage-area-modal .error, #coverage-area-modal .form-error, "
        ...
        raise RuntimeError(f"No cargaron resultados de postal. Mensaje UI: {msg[:200]}")
    except Exception:
        pass

    # 4) si no hay mensaje, tira error claro
    raise PWTimeout("Resultados de postal no se hicieron visibles (delivery-area sigue oculto / sin resultados).")
...
def main():
    ...
    with sync_playwright() as pw:
        ...
        # ✅ BLOQUEAR IMÁGENES (para que no pese tanto)
        def _block_images(route, request):
            try:
                rt = request.resource_type
                u = request.url.lower()
                if rt == "image" or re.search(r"\.(png|jpg|jpeg|webp|gif|svg)(\?|$)", u):
                    return route.abort()
            except Exception:
                pass
            return route.continue_()

        context.route("**/*", _block_images)
