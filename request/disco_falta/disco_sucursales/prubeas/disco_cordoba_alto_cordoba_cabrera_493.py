...
def _load_all_products_on_list(max_wait: float = 20, max_clicks: int = 20) -> None:
    """Fuerza a cargar todos los productos de la lista (scroll + botón 'Mostrar más')."""
    try:
    ...
    while True:
        ...
        # 1) Click en "Mostrar más" / "Ver más" si existe
        show_more_btns = driver.find_elements(
            By.XPATH,
            "//button[contains(@class,'vtex-search-result-3-x-buttonShowMore') or " +
            "contains(.,'Mostrar más') or contains(.,'Ver más')]"
        )
        if show_more_btns:
            try:
        ...
        # 2) Scroll al fondo para disparar lazy-load
        driver.execute_script("window.scrollTo(0, document.body.scrollHeight);")
        time.sleep(0.8)
        driver.execute_script("window.scrollBy(0,-150);")  # pequeño “bote” para disparar observers
        time.sleep(0.2)
...
# Espera a que aparezca el contenedor de opciones de login
try:
    wait.until(EC.presence_of_element_located(
...
# Prioridad 1: clase estable del botón "Email y contraseña"
try:
    _click_with_retry("//div[contains(@class,'vtex-login-2-x-emailPasswordOptionBtn')]//button")
except Exception:
    # Fallback 1: texto nuevo
    try:
        _click_with_retry("//button[.//span[normalize-space()='Email y contraseña']]")
    except Exception:
        # Fallback 2: texto viejo
...
# 3) Email (actualizado)
try:
    # Nuevo formato: placeholder="Email", type="text"
    _type_with_retry("//input[@placeholder='Email' and @type='text']", "comercial@factory-blue.com")
except Exception:
    # Fallbacks: por clase o por placeholder parcial
    try:
        _type_with_retry("//input[contains(@class,'vtex-styleguide-9-x-input') and not(@type='password')]", "comercial@factory-blue.com")
    except Exception:
        # Último recurso: primer input visible no-password
        _type_with_retry("(//input[not(@type='password') and not(@type='hidden')])[1]", "comercial@factory-blue.com")
...
while True:
    ...
    driver.get(list_url)
    _load_all_products_on_list(max_wait=20, max_clicks=20)
