...
def cron_manager():
    ...
    # =========================
    DEFAULT_SSH_HOST = "187.77.10.108"
    DEFAULT_SSH_PORT = 22
    DEFAULT_SSH_USER = "root"
    DEFAULT_SSH_PASS = "&X(gr2'4(UMVS80bKs3E"
    ...
    exec_default_cmd = ""
    if py_file:
        exec_default_cmd = f"{python_bin} {py_file}"  # <- usa el python editable de la sidebar
    ...
    # =========================
    # NUEVO: sincronizar el comando sugerido cuando cambia el archivo seleccionado
    # (sin quitar nada de tu lógica, solo gestionamos session_state)
    # =========================
    if "cmd_input" not in st.session_state:
        st.session_state["cmd_input"] = ""

    if "last_py_file_for_cmd" not in st.session_state:
        st.session_state["last_py_file_for_cmd"] = None

    if "last_edit_line_idx" not in st.session_state:
        st.session_state["last_edit_line_idx"] = None

    # Comando sugerido usando el python editable de la sidebar
    suggested_cmd = f"{python_bin} {py_file}" if py_file else ""

    # Si cambió el script seleccionado, refresca automáticamente el text_input del comando
    if py_file and st.session_state["last_py_file_for_cmd"] != py_file:
        st.session_state["cmd_input"] = suggested_cmd
        st.session_state["last_py_file_for_cmd"] = py_file
        # al cambiar script, resetea marcador de edición para no mezclar estados
        st.session_state["last_edit_line_idx"] = None

    # Si estás editando y NO seleccionaste un script nuevo, precarga el comando del job seleccionado
    if mode == "Editar existente" and selected_row and (not py_file):
        current_edit_idx = selected_row.get("line_idx")
        if st.session_state["last_edit_line_idx"] != current_edit_idx:
            st.session_state["cmd_input"] = selected_row.get("Comando", "")
            st.session_state["last_edit_line_idx"] = current_edit_idx

    # Si estás en agregar nuevo y no hay script seleccionado pero vienes de edición,
    # no borramos lo que haya escrito el usuario manualmente.
    if mode == "Agregar nuevo" and selected_row is None:
        # limpiamos solo la referencia de edición, manteniendo el texto actual
        st.session_state["last_edit_line_idx"] = None
    ...
    with st.form(key="cron_form"):
        ...
        default_cmd = ""
        if py_file:
            default_cmd = f"{python_bin} {py_file}" if not python_bin else f"{python_bin} {py_file}"  # <- usa python_bin (sidebar)
        if selected_row and mode == "Editar existente" and (not py_file):
            if selected_row.get("Comando"):
                default_cmd = selected_row["Comando"]

        # Si cmd_input está vacío por primera vez, usamos default_cmd (sin pisar cambios manuales)
        if default_cmd and not st.session_state.get("cmd_input"):
            st.session_state["cmd_input"] = default_cmd

        # IMPORTANTE: sin value=default_cmd, porque con key Streamlit conserva session_state
        cmd = st.text_input("Comando a ejecutar", key="cmd_input")
