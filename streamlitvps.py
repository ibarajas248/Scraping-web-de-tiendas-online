...
def get_engine():
    ...
    - Si USE_SSH_TUNNEL=1 (default): abre un túnel SSH al VPS y conecta por el puerto local del túnel.
    - Si USE_SSH_TUNNEL=0: conecta directo (para ejecución DENTRO del VPS).
    """
    if USE_SSH_TUNNEL:
        # Túnel SSH como en base_datos.py, pero integrado a SQLAlchemy
        try:
        ...
        engine = create_engine(
            "mysql+mysqlconnector://",  # DSN vacío: usamos creator()
            creator=_creator,
            pool_pre_ping=True,
            pool_recycle=1800,
        )
        ...
        # Conexión directa (para ejecutar dentro del VPS)
        url = (
